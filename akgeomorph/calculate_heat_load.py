...
def calculate_heat_load(area_input, elevation_input, slope_input, aspect_input, conversion_factor, heatload_output):
    """
    ...
    Inputs: 'area_input' -- a raster of the study area to set snap raster and extract area
            ...
            'aspect_input' -- an input 32-bit float aspect raster in degrees
            'conversion_factor' -- an integer to be multiplied with the output for conversion to integer raster
            'heatload_output' -- a file path for an output 16-bit integer heat load index raster
    ...
    grid_extent = Raster(elevation_input).extent
